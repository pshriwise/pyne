...
def cadis(adj_flux_mesh, adj_flux_tag, q_mesh, q_tag,
    ...
    ww_mesh.tag(name = ww_tag, size = num_e_groups, dtype = float, tagtype = 'imesh')
    tag_ww = ww_mesh.get_tag(ww_tag)
    ww_ves = mesh_iterate(ww_mesh.mesh)
    q_bias_mesh.tag(name = q_bias_tag, size = num_e_groups, dtype = float, tagtype = 'imesh')
...
def magic(meshtally, tag_name, tag_name_error, **kwargs):
    ...
    root_tag_name = "{0}_e_upper_bounds".format(meshtally.particle) 
    meshtally.tag(root_tag_name, size = tag_size, dtype = float, tagtype = 'imesh')

    root_tag = meshtally.get_tag(root_tag_name)
