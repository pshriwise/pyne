...
template<> void pyne::_load_data<pyne::gamma>() {
  ...
  for (int i = 0; i < gamma_length; ++i) {
    if ((gamma_array[i].parent_nuc != 0) && !_pyne_isnan(gamma_array[i].energy))
    ...
    for (int j = 0; j < decay_children.size(); ++j) {
      if (nucname::zzzaaa(children[i]) == nucname::zzzaaa(decay_children[j])) {
        ...
        for (int k = 0; k < temp.size(); ++k) {
          if (!_pyne_isnan(temp[k].second) && !_pyne_isnan(temp[k].first)) {
            int found = 0;
...
template<> void pyne::_load_data<pyne::alpha>() {
  ...
  for (int i = 0; i < alpha_length; ++i) {
    if ((alpha_array[i].from_nuc != 0) && !_pyne_isnan(alpha_array[i].energy))
...
template<> void pyne::_load_data<pyne::beta>() {
  ...
  for (int i = 0; i < beta_length; ++i) {
    if ((beta_array[i].from_nuc != 0) && !_pyne_isnan(beta_array[i].avg_energy))
...
template<> void pyne::_load_data<pyne::ecbp>() {
  ...
  for (int i = 0; i < ecbp_length; ++i) {
    if ((ecbp_array[i].from_nuc != 0) && !_pyne_isnan(ecbp_array[i].avg_energy))
